...
def main():
    #---------------------------------------------------
    baseurl = 'https://www.docin.com/p-2118611071.html?toImg=1'
    foldername = '城市轨道交通断面客流估计论文'
    #---------------------------------------------------
    # html = askUrl(baseurl)
    html = askUrlSelenium(baseurl)
...
def getImgurl(html):
    ...
    item = soup.select('#contentcontainer  .panel_inner img' )
    print(item)
    # item = soup.find_all('section', {'id':'page-container','class':'....'} )
    print('正在获取链接')
    linklist = [l['src'] for l in item]
    # for it in item :
    #     print (it)
    # for it in linklist :
    #     print(it)
